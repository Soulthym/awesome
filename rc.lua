...
-- OWN Define your battery widget
battery = battery_widget({adapter = "BAT0", listen = false})
-- OWN Define the volume widget
volumecfg = volume_control({
  step    = '1%'           -- step size for up/down
})
--volumecfg = volume_control({
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "|1) web|", "|2) office|", "|3) code 1|","|4) code 2|" , "|5) code 3|", "|6) code 4|", "|7) code 5|", "|8) config|", "|9) other" }, s, layouts[1])
end
-- }}}
...
for s = 1, screen.count() do
    ...
    right_layout:add(battery.widget) -- OWN
    right_layout:add(volumecfg.widget) --OWN
